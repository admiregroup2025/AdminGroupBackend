...
export const sendOtp = async (req, res) => {
  try {
    ...
    if (!mobileRegex.test(mobile_number.toString())) {
      return res.status(400).json({ msg: 'Invalid mobile number format', success: false });
    }
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (!mobile_number || !otp) {
      return res.status(400).json({ msg: 'Mobile number and OTP are required', success: false });
    }

    // Validate OTP format (should be 4 digits)
    if (!/^\d{4}$/.test(otp.toString())) {
      return res.status(400).json({ msg: 'Invalid OTP format', success: false });
    }
